...
$db_link = mysqli_connect('localhost', '####Datenbank Benutzername####', '####Datenbank Passwort####', '####Datenbank Passwort####');
...
switch ($intent) {
	...
	default:
  $db_res = mysqli_query($db_link, "SELECT * FROM tipps ORDER BY rand() LIMIT 1") or die("Fehler");
  while($row = mysqli_fetch_array($db_res))
  {
    $tipps = $row['tipps'];
  }
    $text = '<speak>'.$tipps.'</speak>';
